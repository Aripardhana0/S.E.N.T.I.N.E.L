"""Worker untuk mengambil ticker & candle dari OKX lalu menyimpannya ke SQLite."""
import logging
...
def fetch_and_store_candles(timeframe: str, limit: int = 200):
    """Ambil candle dari OKX dan simpan ke tabel candles (idempotent)."""
    rows = okx_client.get_candles(config.SYMBOL, bar=timeframe, limit=limit)
    if not rows:
...
def fetch_ticker():
    """Ambil ticker terbaru (dipakai untuk harga real-time / /status)."""
    t = okx_client.get_ticker(config.SYMBOL)
    if t:
        logger.debug("Ticker %s = %s", config.SYMBOL, t.get("last"))
    return t
